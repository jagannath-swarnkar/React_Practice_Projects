...
export class Notes extends Component {
    ...
    UNSAFE_componentWillMount(){
        ...
        // Axios
        // .get('http://localhost:3030/getFiles/'+this.props.todoId)
        // .then((result)=>{
    ...
    noteHandler =()=>{
        ...
        Axios
        .post('http://localhost:3030/note/'+this.props.todoId,({
    ...
    fileSubmit=()=>{
        ...
        Axios
        .post('http://localhost:3030/file-upload/'+this.props.todoId, filedata)
        .then((data)=>{
            ...
            return (
                <div>
                    {this.state.urls.map((url,i)=>{
                        ...
                        return (
                            <ListItem key={i} style={{background:'skyblue', margin:'5px'}} >
                                <a style={{textDecoration:'none',color:'black'}}
                                href={url.file}>File {i+1}</a>
                            </ListItem>
                        )
                    })}
                </div>
        ...
        if(this.props.editNote){
            return(
                <div>
                    ...
                    <div style={{marginTop:'20px',marginBottom:'10px'}} title="Click here to add a file">
          ...
          return(
              <div>
                <div onClick={()=>{this.props.openNote()}}>
                    <Typography title="Click here to add a note"
                        style={{cursor:'pointer',minHeight:'100px'}}>

                        {this.state.note || 'Click here to add a note...'}
                    </Typography>
                </div>
                <div style={{marginLeft:'-40px', display:'flex'}}>
                    <div>
